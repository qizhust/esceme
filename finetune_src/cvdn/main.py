...
from cvdn.agent_hamt_scanme import NavScanmeAgent
...
def main():
    ...
    if args.agent_name in ['seq2seq', 'hamt']:  # HAMT
        agent_class = NavCMTAgent
    elif args.agent_name == 'hamt_scanme':  # ESceme
        agent_class = NavScanmeAgent
    else:
        raise NotImplementedError
    listner = agent_class(args, train_env, rank=rank)

    if not args.test:
        train(args, listner, train_env, val_envs, aug_env=aug_env, rank=rank)
    else:
