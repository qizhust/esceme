...
class CLR2RBatch(R2RBatch):
    def __init__(
        self, feat_db, instr_data:dict, connectivity_dir,
        batch_size=64, c_rate=0.8, angle_feat_size=4,
        seed=0, name=None, sel_data_idxs=None
    ):
        self.env = EnvBatch(connectivity_dir, feat_db=feat_db, batch_size=batch_size)

        self.c_rate = c_rate
        self.curriculum_data = instr_data
        self.data = []
        for _, value in self.curriculum_data.items():
