...
class TDSTPAgent(BaseAgent):
    ...
    def _init_graph(self, batch_size):
        self.graphs = [nx.Graph() for _ in range(batch_size)]  # record the navigable graph up to now
        self.vp2idx_list = [dict() for _ in range(batch_size)]  # record viewpoint_idx, i.e. node_idx in a graph 
        self.seq_idx_list = [list() for _ in range(batch_size)]  # record node idx in the path
        self.seq_vp_list = [list() for _ in range(batch_size)]  # record viewpoint name in the path
        self.seq_view_idx_list = [list() for _ in range(batch_size)]  # record viewpointIndex in the path
        self.seq_dist_list = [list() for _ in range(batch_size)]  # record distance in the path
    ...
    def _get_connectivity_mask(self):
        ...
        for i in range(batch_size):
            adj_matrix = nx.adj_matrix(self.graphs[i], weight=1)  # use the order of self.graphs[i].nodes?
            adj_matrix.setdiag(1)
            adj_matrix = adj_matrix.toarray()
            expanded_matrix = adj_matrix[np.ix_(self.seq_idx_list[i], self.seq_idx_list[i])]  # should guarantee node order
            # node_names_check = list(self.graphs[i].nodes)
            # assert self.vp2idx_list[i][node_names_check[-1]] == self.seq_idx_list[i][-1]  # assertion error??
